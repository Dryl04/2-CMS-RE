...
export async function loadAllThemes(): Promise<PageTheme[]> {
  try {
    ...
    return ((data || []) as PageThemeRow[]).map((row) => ({
      id: row.id,
      name: row.name,
...
export async function saveCustomTheme(theme: PageTheme): Promise<void> {
  try {
    ...
    if (isExisting) {
      ...
      const { error } = await supabase.from("page_themes").insert({
        name: theme.name,
        description: theme.description,
        css: theme.css,
        user_id: user.id,
...
export async function migrateLocalStorageThemes(): Promise<{
  success: boolean;
  count: number;
  message: string;
}> {
