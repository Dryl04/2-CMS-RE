...
export function tokensAreDifferent(
  a: DaisyThemeTokens,
  b: DaisyThemeTokens,
): boolean {
...
export async function loadAllDaisyThemes(): Promise<DaisyTheme[]> {
  const { data, error } = await supabase.from("daisyui_themes").select("*");

  if (error) throw error;

  // Sort themes: light first, dark second, then daisyui themes alphabetically, then custom themes
  const themes: DaisyTheme[] = (data || []) as DaisyTheme[];
...
export async function deleteCustomTheme(id: string): Promise<void> {
  const { error } = await supabase.from("daisyui_themes").delete().eq("id", id);

  if (error) throw error;
}

export function getThemeInlineVars(
  tokens: DaisyThemeTokens,
...
export function generateCustomThemeCSS(
  ...
  if (googleFonts.length > 0) {
    css +=
      generateGoogleFontsImport(googleFonts, DEFAULT_FONT_WEIGHTS) + "\n\n";
  }
 ...
 */
export function extractGoogleFontNames(
  fontConfig?: DaisyFontConfig | null,
): string[] {
  ...
  if (fontConfig.bodyFont) {
    const firstFont = fontConfig.bodyFont
      .split(",")[0]
      .trim()
      .replace(/['"]/g, "");
  ...
  if (fontConfig.headingFont) {
    const firstFont = fontConfig.headingFont
      .split(",")[0]
      .trim()
      .replace(/['"]/g, "");
 ...
 */
export function generateGoogleFontsImport(
  fonts: string[],
...
export async function createCustomThemeWithValidation(
  ...
  const identicalTokens = existingThemes.find(
    (t) =>
      (!excludeIdFromTokensCheck || t.id !== excludeIdFromTokensCheck) &&
      !tokensAreDifferent(t.tokens, tokens),
  );
  if (identicalTokens) {
...
export async function updateCustomThemeWithValidation(
  ...
  if (theme.source !== "custom") {
    throw new ThemeError(
      "Impossible de modifier un thème officiel",
      "FORBIDDEN",
      403,
    );
  }
  ...
  if (updates.slug && updates.slug !== theme.slug) {
    const duplicateSlug = existingThemes.find(
      (t) => t.id !== id && t.slug === updates.slug,
    );
    if (duplicateSlug) {
      throw new ThemeError(
        "Un thème avec ce slug existe déjà",
        "DUPLICATE",
        409,
      );
    }
...
export async function deleteCustomThemeWithValidation(
  ...
  if (theme.source !== "custom") {
    throw new ThemeError(
      "Impossible de supprimer un thème officiel",
      "FORBIDDEN",
      403,
    );
  }
