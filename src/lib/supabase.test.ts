...
test("auth helpers persist and restore a session against the Nest auth routes", async () => {
  ...
  assert.equal(sessionResult.data.session?.user.must_change_password, false);
  ...
  assert.deepEqual(
    calls.map(
      (entry) =>
        `${entry.method ?? "GET"} ${entry.url.replace("http://localhost:3001", "")}`,
    ),
    ["POST /auth/login", "GET /auth/me", "GET /auth/me", "POST /auth/logout"],
  );
});

test("password change updates the stored session and clears the must-change flag", async () => {
  storage.setItem("cms_api_access_token", "token-legacy");
  storage.setItem(
      ...
      const url =
        typeof input === "string"
          ...
          : input instanceof URL
            ? input.toString()
            : input.url;
      assert.equal(url, "http://localhost:3001/auth/change-password");
  ...
  const result = await supabase.auth.changePassword({
    currentPassword: "ChangeMe123!",
    newPassword: "StrongPass456!",
  });

  assert.equal(result.error, null);
  assert.equal(result.data.user?.must_change_password, false);
  assert.equal(storage.getItem("cms_api_access_token"), "token-updated");
});
...
test("public seo page queries map Nest payloads back to the legacy snake_case shape", async () => {
  ...
  assert.deepEqual(result.data?.sections_data, [
...
test("media upload compatibility bridges storage upload and legacy media_files inserts", async () => {
  ...
  const uploadResult = await supabase.storage
    .from("media")
    .upload("user-1/file.png", file);
  assert.equal(uploadResult.error, null);

  const publicUrlResult = supabase.storage
    .from("media")
    .getPublicUrl("user-1/file.png");
  assert.equal(
    publicUrlResult.data.publicUrl,
    "https://cdn.example.com/user-1/file.png",
  );
