...
export default function ProcessStepsCardsWidget({ section }: ProcessStepsCardsWidgetProps) {
  ...
  return (
    <section className="bg-base-100">
      ...
      >
        <h2 className="text-3xl sm:text-4xl font-bold mb-2" style={headingColor ? { color: headingColor } : undefined}>
          {content.title || 'Create at the speed of thought.'}
        </h2>
        <p className="text-sm opacity-80 mb-8" style={textColor ? { color: textColor } : undefined}>
        ...
        <div className="grid md:grid-cols-3 gap-6">
          {steps.map((step: ProcessStepItem, index: number) => (
            <article key={index} className="bg-base-200 rounded-2xl border border-base-content/10 overflow-hidden">
              ...
              <div className="p-4">
                <p className="text-sm font-semibold opacity-80">{step.number || `0${index + 1}`}</p>
                <h3 className="text-xl font-bold mt-1" style={headingColor ? { color: headingColor } : undefined}>{step.title || 'Step'}</h3>
                <p className="text-sm mt-2 opacity-80" style={textColor ? { color: textColor } : undefined}>{step.description || ''}</p>
              </div>
            </article>
          ))}
        </div>
