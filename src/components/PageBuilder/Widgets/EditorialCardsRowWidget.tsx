    ...
    <section className="bg-base-100">
      ...
      >
        <div className="flex items-end justify-between mb-6">
          ...
          {content.ctaText && <a href={content.ctaLink || '#'} className="btn btn-outline btn-sm">{content.ctaText}</a>}
        </div>
        ...
        <div className={`grid sm:grid-cols-2 ${columns} gap-4`}>
          {cards.map((card: EditorialCard, index: number) => (
            <article key={index} className="rounded-2xl border border-base-content/10 overflow-hidden bg-base-200">
              ...
              <div className="p-4">
                <h3 className="text-lg font-semibold" style={headingColor ? { color: headingColor } : undefined}>{card.title || 'Card title'}</h3>
                <p className="text-sm mt-2 opacity-80" style={textColor ? { color: textColor } : undefined}>{card.description || ''}</p>
                {card.meta && <p className="text-xs mt-3 opacity-60">{card.meta}</p>}
              </div>
            </article>
          ))}
        </div>
